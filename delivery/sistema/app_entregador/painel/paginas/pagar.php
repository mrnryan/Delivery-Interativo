...
//totalizar páginas
if($pago == 'Vencidas'){
	$query2 = $pdo->query("SELECT * from pagar where vencimento < curDate() and pago != 'Sim' order by id desc");
}else{
	$query2 = $pdo->query("SELECT * from pagar where vencimento >= '$dataInicial' and vencimento <= '$dataFinal' and pago LIKE '%$pago%' order by id desc");
}
...
if($pago == 'Vencidas'){
	...
	$query = $pdo->query("SELECT * from pagar where vencimento >= '$dataInicial' and vencimento <= '$dataFinal' and pago LIKE '%$pago%' order by id desc LIMIT $limite, $itens_pag");
}
...
//PEGAR RESIDUOS DA CONTA
	...
	$query2 = $pdo->query("SELECT * FROM pagar WHERE id_ref = '$id' and residuo = 'Sim'");
	$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
	if(@count($res2) > 0){
...
if($fornecedor != 0 || $funcionario != 0){
	...
	if($total_reg2 > 0){
		...
		$pix_pessoa = '';
					...
					<div class="post_entry" onclick="mostrar('{$descricao}','{$valorF}','{$nome_pessoa}','{$vencimentoF}','{$data_pgtoF}','{$nome_pgto}','{$nome_frequencia}','{$obs}','{$tumb_arquivo}','{$multaF}','{$jurosF}','{$descontoF}','{$taxaF}','{$subtotalF}','{$nome_usu_lanc}','{$nome_usu_pgto}', '{$pago}', '{$arquivo}', '{$telefone_pessoa}', '{$pix_pessoa}', '{$tipo_pessoa}')">
					...
					<p class="subtitulo_list"><span class="{$ocultar_cliente}">{$nome_pessoa} / </span>Venc: {$vencimentoF} </p>
					</div>
					...
					<a onclick="editar('{$id}','{$descricao}','{$valor}','{$fornecedor}','{$funcionario}','{$vencimento}','{$data_pgto}','{$forma_pgto}','{$frequencia}','{$obs}','{$tumb_arquivo}')" style="width: 16.66%; background: #3c6cb5" href="#" class="action1"><img src="images/icons/white/edit.png" alt="" title="" /></a>
		...
		<div class="page_single layout_fullwidth_padding">
			<div class="contactform">
				<form id="form" method="post">
					...
					<div style="">
						<label class="labels_form">Fornecedor</label>
						<div class="selector_overlay">
							<select name="fornecedor" id="fornecedor" class="sel2" style="height:40px !important;">
					...
					<div style="">
						<label class="labels_form">Funcionário</label>
						<div class="selector_overlay">
							<select name="funcionario" id="funcionario" class="sel2" style="height:40px !important;">
								<option value="0">Selecionar Funcionário</option>
								<?php 
								$query = $pdo->query("SELECT * from usuarios where nivel != 'Administrador' and nivel != 'Cliente' order by id asc");
								$res = $query->fetchAll(PDO::FETCH_ASSOC);
								$linhas = @count($res);
								if($linhas > 0){
									for($i=0; $i<$linhas; $i++){
					...
					<div style="width:48%; float:right">
						...
						<input type="date" name="vencimento" id="vencimento" value="<?php echo $data_hoje ?>" class="form_input_data"  style="height:25px; "/>
					</div>



						<div style="width:48%; float:right; margin-top: -7px">
						<label class="labels_form">Pago em</label>
						<input type="date" name="data_pgto" id="forma_pgto" value="" class="form_input_data"  style="height:25px;"/>
					</div>
...
<div class="popup popup-mostrar" style="width:85%; margin:20px;  background: transparent; margin-top: 60px; ">
	<div class="content-block">
		...
		<ul class="responsive_table tabela_dados" style="height:450px !important; overflow: scroll !important; height:100%; scrollbar-width: thin;">
			...
			<li class="table_row celula_tabela">
				<div class="table_section" style="width:35%">Pessoa</div>
