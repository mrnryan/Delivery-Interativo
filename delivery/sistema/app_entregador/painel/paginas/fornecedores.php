...
//totalizar páginas
$query2 = $pdo->query("SELECT * from fornecedores where nome like '%$buscar%' or telefone like '%$buscar%' or email like '%$buscar%' order by id desc");
...
<div id="pages_maincontent" style="background: #FFF; ">
	...
	<div class="loginform" style="margin-top: 10px">
		<form method="post">
		<input  type="text" name="buscar" id="buscar" value="<?php echo $buscar ?>" class="form_input required" placeholder="Buscar Fornecedores" style="background: transparent !important; width:80%; float:left" />
	...
	<div class="page_single layout_fullwidth_padding"> 
		...
		<ul class="posts2" id="listar">
			<?php 
			$query = $pdo->query("SELECT * from fornecedores where nome like '%$buscar%' or telefone like '%$buscar%' or email like '%$buscar%' order by id desc LIMIT $limite, $itens_pag");
	...
	$tel_whatsF = '55'.preg_replace('/[ ()-]+/' , '' , $telefone);
					...
					<div class="post_entry" onclick="mostrar('{$nome}','{$email}','{$telefone}','{$endereco}','{$pix}','{$dataF}')">
					...
					<a target="_blank" style="width: 25%; background: #1d8c32" href="http://api.whatsapp.com/send?1=pt_BR&phone={$tel_whatsF}" title="Whatsapp"><img src="images/icons/white/whatsapp.png" alt="" title="" /></a>
					<a style="width: 25%; background: #234e9e" href="#" onclick="arquivo('{$id}','{$nome}')" title="Inserir Arquivo"><img src="images/icons/white/arquivo.png" alt="" title="" /></a>
					<a onclick="excluir_reg('{$id}', '{$nome}')" style="width: 25%; background: #d4412a" href="#"><img src="images/icons/white/trash.png" alt="" title="" /></a>
					<a onclick="editar('{$id}','{$nome}','{$email}','{$telefone}','{$endereco}','{$pix}')" style="width: 25%; background: #3c6cb5" href="#" class="action1"><img src="images/icons/white/edit.png" alt="" title="" /></a>
			...
			<div class="contactform">
				<form id="form" method="post">
					...
					<div style="width:48%; float:right">
						...
						<input type="text" name="pix" id="pix" value="" class="form_input" style="height:25px;"/>
					</div>
