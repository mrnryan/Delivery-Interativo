...
//totalizar páginas
$query2 = $pdo->query("SELECT * from formas_pgto where nome like '%$buscar%' order by id desc");
...
<div id="pages_maincontent" style="background: #FFF; ">
	...
	<div class="loginform" style="margin-top: 10px">
		<form method="post">
		<input  type="text" name="buscar" id="buscar" value="<?php echo $buscar ?>" class="form_input required" placeholder="Buscar Cargos" style="background: transparent !important; width:80%; float:left" />
	...
	<div class="page_single layout_fullwidth_padding"> 
		...
		<ul class="posts2" id="listar">
			<?php 
			$query = $pdo->query("SELECT * from formas_pgto where nome like '%$buscar%'  order by id desc LIMIT $limite, $itens_pag");
	...
	if($taxa == ""){
		$taxa = 0;
					...
					<a onclick="editar('{$id}','{$nome}','{$taxa}')" style="width: 45px; height:45px; background: #3c6cb5" href="#" class="action1"><img src="images/icons/white/edit.png" alt="" title="" style="margin-top:15px"/></a>
			...
			<div class="contactform">
				<form id="form" method="post">
					...
					<div style="width:48%; float:right">
						<label class="labels_form">Taxa %</label>
						<input type="number" name="taxa" id="taxa" value="" class="form_input" style="height:25px;"/>
					</div>
